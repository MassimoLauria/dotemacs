...
#+END_SRC
   ...
   The list of recent  files is saved on Emacs exit  by default. I use
   Emacs as a server process,  therefore it usually exits abruptly not
   saving the recent files. Let's save them every few minutes.
...
(use-package recentf
    ...
    :init 
    (run-at-time nil (* 3 60) 'recentf-save-list/silent)  ; three minutes
    )
#+END_SRC
