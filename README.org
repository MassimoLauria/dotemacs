...
(use-package dashboard
  ...
  (setq dashboard-items '((recents   . 8) (bookmarks . 8)))
  (setq dashboard-hide-cursor nil)
...
(use-package vertico
  ...
  (vertico-mode)
  (marginalia-mode)
  (keymap-set vertico-map "M-SPC" #'embark-act-with-completing-read)
  )
...
** Looking up stuff in dictionaries
...
(use-package sdcv
  ...
  (defun sdcv-search-completion (&optional word)
    ...
    (sdcv-search-input (or ;word
                           (word-at-point)
                           (completing-read "Lookup in dictionary: " sdcv-word-cache)
                           ))))
#+END_SRC
...
Fist let's tell ~citar~ where is the bibtex database and the files.
