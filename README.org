...
a very complete way, mostly because the IMAP interface that gmail
exposes is insufficient for many good features of GMAIL. For example
tagging via notmuch is not synced between machine and that's
a serious issues.
...
Drafts are stored in hardcoded path '~/personal/mail/drafts/'
with timestamped filename.
"
  (interactive)
  (let ((buffer)
        (spos)
        (loc "~/personal/mail/drafts/")
