...
(use-package citar
   ...
   :custom
   (citar-bibliography '("~/lavori/latex/bibliografia.bib"))
...
The ~compile~ and ~recompile~ commands are useful starting points.
The main issue I have with those is that they are not launched from
the top level project directory. I run compilation commands using
~compile-multi~ and among other things, this package allows me to
do that.

Hence we first do the least possible setup for ~projectile~. A remark:
I prefer to use build using the innermost Makefile from the current
directory, in case there are many. This is not necessarily a universal
good thing but works for some of my course projects.
...
(use-package projectile
  ...
  (add-to-list 'projectile-project-root-files-bottom-up ".latexmkrc")
  (projectile-mode))
#+END_SRC
...
(use-package compile-multi
  ...
  :config
  (push `(t
          ("Fallback:Edit command" . ,#'mxl/compile-multi-read))
        compile-multi-config))

#+END_SRC

Here we have an example on how to add compile actions. If the project
...
(push `(latex-mode
        ("Latex:LaCheck" "lacheck" (concat (TeX-master-file) ".tex"))
        ("Latex:ChkTeX"  "chktex -v6"  (concat (TeX-master-file) ".tex")))
        compile-multi-config)
#+END_SRC

To "build" org files there is the ~org-export-dispatch~ command. I can
bind it directly to F9 but I want to experiment the integration with
~compile-multi~. Maybe it will just be one more key to press most of
the time, but it is useful when the org file is inside some project
that has more build options, that would be unavailable when
~org-export-dispatch~ is bound to F9.
...
*** Automatic loading of Makefile target
...
** Fixing Errors
   ...
   - =F11= previous error;
   - =F12= next error.
   And I use =M-F11= and =M-F12= to toggle the compilation windows.
   To do the latter we need to implement specific functions.
...
In order to toggle the visibility of the compilation buffer I have to
write specific code. This is annoying and moreover does not cover many
...
(global-set-key (kbd "M-<f11>") #'toggle-compilation-window)
(global-set-key (kbd "M-<f12>") #'toggle-compilation-window)
#+END_SRC


Most programming languages are supported by =flycheck=, a tool that
automatically checks the buffer for error while writing. It =flycheck=
activates in the buffers to the supported languages, assuming all
needed tools for that language are present.
Use =flycheck-verify-setup= to investigate the checkers
enabled/disabled/available in a buffer.

I used to care about having errors/warning/ and stuff highlighted
while I write. But I believe that's a bit annoying and distracting.
Now I tentatively allow highlight of actual errors during typing.
Therefore I have a minimal setup of ~flycheck~. Furthermore I forbid
such tools to hijack =F11= and =F12=.
...
(push `(c++-mode
        ("C++ programming:compile"
         "g++" (buffer-file-name) "-Wall -Werror" "-g"  "-o"
...
(push `(go-mode
        ...
        ("Go file:format" "gofmt"    (file-relative-name (buffer-file-name)))
