...
To have auto completion I have  a simple setup based on ~corfu~, which
complements ~vertico~,  which fits  the same  role in  the minibuffer.
Contrary  to tools  like ~company~,  ~corfu~ uses  the same  logic and
technology of ~vertco~ (e.g. ~orderless~ matching of candidates).
This setup is very basic but functional.
