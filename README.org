...
(defvar mxl/pinned-compilation-directory nil
  "If not nil, complie-multi will always launch compilation commands from here.")
...
(defvar mxl/eglot-blacklist-paths (list "~/")
  "Never activate eglot if the project root is in any of these folders")

(defun mxl/eglot-ensure-blacklist ()
  "Activate eglot only if the path is not in
`mxl/eglot-blacklist-paths'"
  (interactive)
  (let ((all-different t))
    (dolist (element mxl/eglot-blacklist-paths all-different)
      (when (and (stringp element) (string= (expand-file-name default-directory)
                                            (expand-file-name element)))
        (setq all-different nil)))
    (if all-different
      (eglot-ensure)
      (message "Eglot blacklisted in %s" default-directory))))
...
(use-package go
  ...
  :hook ((go-mode . mxl/eglot-ensure-blacklist)
...
(use-package python
  ...
  :hook ((python-mode . mxl/eglot-ensure-blacklist))
