...
** Trailing whitespaces
  ...
  But I  code this  is not  usually an issue.  So let's  set automatic
...
** Python
   ...
   - requires to run ~pip install python-language-server~ to instal =pyls=;
...
* Writing facilities
  ...
  In  my  configuration template  expansion  activates  using an  Helm
  selection, or  by completion  after a character  ='<'= which  is the
  first character in  all yasnippet keys. For this reason  I don't use
  =yasnippet-snippets= because it fills  by typing space with snippets
  which trigger out of the blue.
  
** Completion

   I like  to move  in the  completion menu  using the  =M-jkli= keys.
   =TAB= activates completion, and =M-o= only expands the common part.
...
(use-package yasnippet
  ...
  :config
  (add-to-list 'yas-snippet-dirs mxl-local-snippet-dir)
  (progn
    (yas-reload-all)))
#+END_SRC
...
** Choice and expansion of templates (prefix and completion)

   I have been a  user of =tempo= template system and  I like the idea
   of  having an  explicit prefix  key to  trigger templates.  I never
   liked template expansion to pop up  while I am writing other words.
   For example  in =yasnippet-snippets= there is  a template triggered
   by  the letter  ='a'=. A  bit too  agressive IMHO.  Instead I  will
   trigger them only with that prefix.

   As in  =tempo= templates,  I use  ='<'= to  trigger the  choice and
   expansion of templates. 
#+BEGIN_SRC emacs-lisp
(defcustom mxl-template-trigger-key "<"
  "Keys that trigger choice and expansion of templates")
#+END_SRC

   While  I don't  have  =company-yasnippet=  among my  =company-mode=
   backends, I can still use it on demand
   
#+BEGIN_SRC emacs-lisp
(use-package company-yasnippet
  :commands company-yasnippet)

(defun mxl-complete-template ()
  "Open company selector for yasnippet templates.

Call `company-yasnippet' after the insertion of the
`mxl-template-trigger-key' character. This solution is easier
than fighting to integrate `company-mode' with
`company-yasnippet'."
  (interactive)
  (insert mxl-template-trigger-key)
  (call-interactively 'company-yasnippet))
#+END_SRC

   Activate the template completion menu.

#+BEGIN_SRC emacs-lisp
(global-set-key (kbd mxl-template-trigger-key)
                'mxl-complete-template)
#+END_SRC
