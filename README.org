...
(setq default-font-height
      (cond
        ((string= system-name "lauriatx") 240) ;; My laptops have smaller screens
        ((string= system-type "darwin"  ) 240)
        (t 180)))
...
(use-package doom-modeline
  ...
  (setq doom-modeline-major-mode-icon nil)  ;; useless
...
(defun mxl/toggle-mode-line ()
  ...
  (interactive)
  (if (and (local-variable-p 'mode-line-format)
...
Indeed with ~olivetti-mode~ it would be a good idea to hide
the modeline.

#+BEGIN_SRC emacs-lisp
(use-package olivetti
  :hook ((olivetti-mode-on  .  mxl/hide-mode-line)
         (olivetti-mode-off  . mxl/show-mode-line)))
#+END_SRC
...
I often do the same spelling mistake over and over and of course
=abbrev-mode= helps here automatically fixing those.

#+BEGIN_SRC emacs-lisp
(use-package abbrev
  :defer 1
  :custom
  (abbrev-file-name (expand-file-name "abbrev_defs" base-config-path))
  (abbrev-mode 1)
  :config
...
(use-package cc-mode
  ...
  :bind (:map c-mode-map
              ("RET" . c-context-line-break))  ;; the proper line break
