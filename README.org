...
** Fonts 
   ...
   The  default  font is  included  with  this configuration,  but  for
   windows  (which  I   almost  never  use)  I   have  an  alternative.
   Notice that on X11 we can also use ~Xresources~ file.
...
(if (eq system-type 'windows-nt)
 ...
 #+END_SRC
   ...
   Text is more readable with some space between lines.

 #+BEGIN_SRC emacs-lisp
(setq line-spacing 0.2)
 #+END_SRC

   I like Emacs to open in a wide frame at the center of the screen, on
   startup.  At  least  on  these  systems  where  the  layout  of  the
   application is left  to the user. A very small  internal border puts
   distance between  the characters and  screen borders, and  all other
   decorations are deactivated.
...
** Themes
   ...
   Setting for the appearance of emacs. 
